...
let
  ...
  configFile = (pkgs.formats.toml { }).generate "aerospork.toml" settings;
in
{
  environment.systemPackages = [ aerospork ];

  launchd.user.agents.aerospork = {
    command = "${aerospork}/Applications/AeroSpork.app/Contents/MacOS/AeroSpork --config-path ${configFile}";
    serviceConfig = {
      KeepAlive = true;
      RunAtLoad = true;
    };
